...
#used to find coordinates of change in the top of the image
...
def findRatioTop(image):
    pointsOfChangeTop = getPointsOfChangeTop(image)
    print(pointsOfChangeTop)
    angleImage, angle = findAngle(image, getPointsOfChangeTop(image), getPointsOfChangeBottom(image))


    return pointsOfChangeTop[0]/pointsOfChangeTop[1], pointsOfChangeTop[0]/image.shape[1], pointsOfChangeTop[1]/image.shape[1]

def findRatioBottom(image):
    pointsOfChangeBottom = getPointsOfChangeBottom(image)
    print(pointsOfChangeBottom)
    angleImage, angle = findAngle(image, getPointsOfChangeTop(image), getPointsOfChangeBottom(image))
    return pointsOfChangeBottom[0]/pointsOfChangeBottom[1], pointsOfChangeBottom[0]/image.shape[1], pointsOfChangeBottom[1]/image.shape[1]
...
def main():
    ...
    cannyImage = canny(imageCopy)

    cv2.imshow("test", cannyImage)
    cv2.waitKey(0)

    print("ratio: " + str(findRatioTop(cannyImage)))
    print("ratio: " + str(findRatioBottom(cannyImage)))
    print("pixel lenght: " + str(cannyImage.shape[0]))
    print("pixel width: " + str(cannyImage.shape[1]))
